...
public class SpringScrollView extends ReactViewGroup implements View.OnTouchListener,
    ...
    protected void onAttachedToWindow() {
        ...
        if (child != null) {
            if (initContentOffset.y != 0) {
                ...
                contentOffset.x = initContentOffset.x;
                contentOffset.y = initContentOffset.y;
                View child0 = getChildAt(0);
                if (child0 != null) {
                    child0.setTranslationX(-contentOffset.x);
                    child0.setTranslationY(-contentOffset.y);
                }
            }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if(initHeight > 0 && initHeight > h){
            h = initHeight;
        }
        setContentSize(child.getMeasuredWidth(), h);
    }
    ...
    public void setInitHeight(float height) {
        initHeight = height;
    }
